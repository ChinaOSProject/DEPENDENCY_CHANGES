...
- Reason: npm `10.9.9` 的 `engines.node` 要求为 `^18.17.0 || >=20.5.0`；目标系统基座为 Debian 10 / glibc 2.28。
- Root Cause: Debian 10 系统提供的 Node.js 版本无法满足 npm `10.9.9` 的运行要求。
...
- Project-side Changes: 无源码修改；生成 `npm_v10.9.9_all.deb`。
